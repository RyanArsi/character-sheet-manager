    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">Nova Campanha</h3>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">Minhas Campanhas</h3>

                    @if($campaigns->isEmpty())
                        <p class="text-gray-500 text-sm">Você ainda não participa de nenhuma campanha. Crie uma acima ou entre por um link de convite.</p>
                    @else
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <a href="{{ route('campanhas.ver', $campaign) }}"
                                ...
                                <div class="flex items-center justify-between">
                                    <p class="font-medium text-gray-800 group-hover:text-amber-700 truncate">{{ $campaign->name }}</p>
                                    @if($campaign->owner_id === auth()->id())
                                        <span class="text-xs font-medium text-amber-700 bg-amber-50 rounded-full px-2 py-0.5">Mestre</span>
                                    @else
                                        <span class="text-xs font-medium text-gray-500 bg-gray-100 rounded-full px-2 py-0.5">Jogador</span>
                                    @endif
                                </div>
                                ...
                                @if($campaign->description)
                                    <p class="text-sm text-gray-600 mt-2 line-clamp-2">{{ $campaign->description }}</p>
                                @endif
                            </a>
                            @endforeach
                        </div>
