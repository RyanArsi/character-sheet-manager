<div>
    {{-- ============ LISTA (talentos da ficha) ============ --}}
    @if($view === 'list')
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-sm font-bold text-amber-500 uppercase tracking-widest">Talentos</h2>
            <div class="flex items-center gap-2">
                {{-- Engrenagem: configura o que acontece ao usar um talento (compartilha config com jutsus) --}}
                <div class="relative" x-data="{ cfgOpen: false }">
                    <button type="button" @click="cfgOpen = !cfgOpen" dusk="talent-config"
                    ...
                    <div x-show="cfgOpen" x-cloak @click.outside="cfgOpen = false"
                        ...
                        <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Ao usar o talento</p>
                        <label class="flex items-center gap-2 text-xs text-gray-300 cursor-pointer">
                            <input type="checkbox" x-model="jutsuCfg.chakra" dusk="tcfg-chakra"
                                class="rounded border-gray-600 bg-gray-900 text-amber-500 focus:ring-0 focus:ring-offset-0">
                            Descontar chakra
                        </label>
                        <label class="flex items-center gap-2 text-xs text-gray-300 cursor-pointer">
                            <input type="checkbox" x-model="jutsuCfg.test" dusk="tcfg-test"
                                class="rounded border-gray-600 bg-gray-900 text-amber-500 focus:ring-0 focus:ring-offset-0">
                            Rolar teste
                        </label>
                        <label class="flex items-center gap-2 text-xs text-gray-300 cursor-pointer">
                            <input type="checkbox" x-model="jutsuCfg.damage" dusk="tcfg-damage"
                                class="rounded border-gray-600 bg-gray-900 text-amber-500 focus:ring-0 focus:ring-offset-0">
                            Rolar dano
                        </label>
                    </div>
        ...
        @forelse($assigned as $talent)
            @include('livewire.partials.talent-card', ['talent' => $talent, 'mode' => 'assigned', 'authId' => $authId])
        @empty
            <p class="text-gray-600 text-sm">Nenhum talento nesta ficha ainda. Use <span class="text-gray-400">Biblioteca</span> para atribuir ou <span class="text-gray-400">Criar</span> um novo.</p>
        @endforelse
    @endif
    ...
    @if($view === 'browse')
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-sm font-bold text-amber-500 uppercase tracking-widest">Biblioteca de talentos</h2>
        ...
        <div class="flex flex-wrap items-center gap-2 mb-4 pb-3 border-b border-gray-700">
            <button type="button" wire:click="exportTalents" dusk="talent-export"
                class="px-2.5 py-1 text-[11px] font-medium rounded bg-gray-700 hover:bg-gray-600 text-gray-200 transition-colors">
                ↓ Exportar JSON
            </button>
            ...
            <label class="px-2.5 py-1 text-[11px] font-medium rounded bg-gray-700 hover:bg-gray-600 text-gray-200 cursor-pointer transition-colors">
                ↑ Escolher arquivo
                <input type="file" class="hidden" wire:model="importFile" accept=".json,application/json" dusk="talent-import-file">
            </label>

            @if($importFile)
                <button type="button" wire:click="importTalents" wire:loading.attr="disabled" wire:target="importTalents,importFile"
                    dusk="talent-import"
        ...
        @forelse($available as $talent)
            @include('livewire.partials.talent-card', [
                'talent' => $talent,
                'mode' => in_array($talent->id, $assignedIds, true) ? 'in-sheet' : 'available',
                'authId' => $authId,
            ])
        @empty
            <p class="text-gray-600 text-sm">Nenhum talento encontrado{{ $activeFilters ? ' para essas tags' : '' }}.</p>
        @endforelse
    @endif
    ...
    @if($view === 'form')
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-sm font-bold text-amber-500 uppercase tracking-widest">
                {{ $editingId ? 'Editar talento' : 'Novo talento' }}
            </h2>
        ...
        <form wire:submit="save" class="space-y-3">
            ...
            {{-- Áudio/Vídeo (toca ao usar o talento) --}}
            <div>
                ...
                <div class="flex items-center gap-3">
                    <label class="px-2.5 py-1 text-[11px] font-medium rounded bg-gray-700 hover:bg-gray-600 text-gray-200 cursor-pointer transition-colors flex-shrink-0">
                        ...
                        <input type="file" class="hidden" wire:model="media" accept="audio/*,video/*" dusk="talent-media">
                    </label>
                    @if($media)
                ...
                <div class="flex items-center gap-2 mt-2" x-data="{ vol: @js((int) $volume) }">
                    ...
                    <input type="range" min="0" max="100" dusk="talent-volume"
                        x-model.number="vol"
            ...
            <div>
                ...
                <input type="text" wire:model="name" dusk="talent-name"
            ...
            <div>
                ...
                <input type="text" wire:model="tagsInput" placeholder="ex.: suporte, passivo, físico"
                    class="w-full bg-gray-900 border border-gray-700 rounded px-2 py-1.5 text-white text-sm focus:border-amber-500 focus:ring-0 focus:outline-none">
            </div>
            ...
            <div class="flex items-center justify-between pt-1">
                @if($editingId)
                    <button type="button" wire:click="deleteTalent({{ $editingId }})"
                        wire:confirm="Excluir este talento? Ele será removido de todas as fichas."
